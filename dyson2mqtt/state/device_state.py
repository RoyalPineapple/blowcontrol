...
class DeviceStateListener:
    ...
    def _mqtt_callback(self, client, userdata, msg):
        ...
        print(f"[DeviceStateListener] DEBUG: Received message on {msg.topic}: {msg.payload}")
        try:
            ...
            with self._lock:
                if msg_type == "CURRENT-STATE":
                    ...
                    print(f"[DeviceStateListener] Updated current state")
                elif msg_type == "STATE-CHANGE":
                    ...
                    if self._current_state and "product-state" in data:
                        ...
                        self._last_update = time.time()
                        print(f"[DeviceStateListener] Applied state change")
                elif msg_type == "ENVIRONMENTAL-CURRENT-SENSOR-DATA":
                    self._environmental_data = data
                    self._last_update = time.time()
                    print(f"[DeviceStateListener] Updated environmental data")
                else:
            ...
            def listener_worker():
                try:
                    ...
                    def delayed_request():
                        ...
                        time.sleep(3)
                        print("[DeviceStateListener] Sending REQUEST-CURRENT-STATE...")
                        try:
                            self._client.send_command("REQUEST-CURRENT-STATE")
                        except Exception as e:
                            print(f"[DeviceStateListener] Error sending request: {e}")
                    
                    # Start the delayed request in a separate thread
                    request_thread = threading.Thread(target=delayed_request, daemon=True)
