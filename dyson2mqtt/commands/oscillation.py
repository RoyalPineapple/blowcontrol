...
def set_oscillation_angles(width: Union[int, str], heading: Union[int, str] = 180) -> dict:
    """
    ...
    if needs_adjustment:
        ...
        if width_int > 350:  # 355 - 5 = 350° maximum possible width
            raise ValueError(f"Width {width_int}° is too large. Maximum width is 350° (5° to 355°).")
        ...
        if not valid_options:
            raise ValueError(f"Width {width_int}° cannot fit within Dyson's bounds (5°-355°) from any heading. Try a smaller width.")
        
        # Choose the closest valid option
        best_heading, best_distance, adjustment_type = min(valid_options, key=lambda x: x[1])
        ...
        logger.warning(f"Adjusted heading from {original_heading}° to {heading_int}° to fit width {width_int}° within bounds (adjusted {adjustment_type})")
    
    # Final validation - this should always pass now
    if lower_angle > upper_angle or lower_angle < 5 or upper_angle > 355:
        raise ValueError(f"Internal error: failed to find valid heading for width {width_int}°")
    
    # Handle wrap-around case (e.g., 350° to 10°) - only allowed if within bounds
    if lower_angle > upper_angle:
        logger.warning(f"Wrap-around oscillation: {lower_angle}° to {upper_angle}° (width={width_int}°, heading={heading_int}°)")
    
    logger.info(f"Setting oscillation: width={width_int}°, heading={heading_int}° -> angles {lower_angle}°-{upper_angle}°")
    
    # Debug output
    if original_heading != heading_int:
        logger.info(f"Heading was adjusted from {original_heading}° to {heading_int}° (lower: {lower_angle}°, upper: {upper_angle}°)")
    ...
    try:
        ...
        success = client.send_standalone_command("STATE-SET", command_data)
        if success:
            logger.info(f"✅ Oscillation set: {lower_angle}°-{upper_angle}° (width={width_int}°, heading={heading_int}°)")
        else:
    ...
    if isinstance(width_input, str):
        ...
        if width_lower in WIDTH_NAMES:
            ...
            try:
                ...
                raise ValueError(f"Invalid width name '{width_input}'. Valid names: {available_names}")
...
def set_oscillation_width(width_input, fallback_heading: int = 180) -> dict:
    ...
    if width not in VALID_WIDTHS:
        ...
        logger.warning(f"Width {width}° is not a valid Dyson step. Using {valid_width}° ({width_name}) instead. Valid: off, narrow, medium, wide, full")
        width = valid_width
        ...
        if state_result and state_result.get("state"):
            ...
            if product_state:
                if "osal" in product_state and "osau" in product_state:
                    ...
                    logger.info(f"Estimated current position from oscillation range: {current_heading}° (range: {osal}°-{osau}°)")
            ...
            if current_heading is None:
                ...
                if product_state:
                    logger.debug(f"Available product-state keys: {list(product_state.keys())}")
    ...
    if current_heading is None:
        ...
        logger.warning(f"Using fallback heading {fallback_heading}° (could not determine current position)")
    
    # Use current or fallback position as heading and set the oscillation
    width_name = WIDTH_DISPLAY_NAMES.get(width, f"{width}°")
    logger.info(f"Setting {width}° ({width_name}) width centered on position {current_heading}°")
...
def set_oscillation_direction(heading: Union[int, str]) -> dict:
            ...
            if product_state:
                ...
                if oscs == "ON" and oson == "ON":
                    ...
                    if "osal" in product_state and "osau" in product_state:
                        ...
                        logger.info(f"Current oscillation: {osal}°-{osau}° (width: {current_width}°)")
                    else:
                        logger.warning("Oscillation is on but no angle data found")
                else:
                    logger.info("Oscillation is currently off - will just set heading without turning oscillation on")
    ...
    # If oscillation is off, use the sweep pattern to set heading without turning oscillation on
    if not oscillation_is_on:
        logger.info(f"Oscillation is off - setting heading to {heading_int}° using sweep pattern")
        ...
        try:
            ...
            if success:
                logger.info(f"✅ Set heading to {heading_int}° (oscillation remains off)")
                return {
                    "success": True,
                    "actual_width": 0,
                    "actual_heading": heading_int,
    ...
    if current_width is None:
        ...
        logger.warning(f"Using default width {current_width}° (could not determine current width)")
        width_preserved = False
    else:
    ...
    if current_width not in VALID_WIDTHS:
        ...
        logger.warning(f"Current width {current_width}° is not a valid step, using closest: {closest_width}°")
    ...
    logger.info(f"Setting heading to {heading_int}° while preserving {current_width}° ({width_name}) width")
